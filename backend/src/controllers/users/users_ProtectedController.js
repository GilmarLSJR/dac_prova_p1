...
class Users_ProtectedController {

    // Método para listar todos os usuários
    static async adminOnly_getAll(req, res) {
        console.log('users_ProtectedController_adminOnly_getAll');
        try {
            const users = await usersService.getAll(); // Chama o service para buscar usuários
    ...
    static async getByID(req, res) {
        console.log('users_ProtectedController_getByID');
        try {
            ...
            const users = await usersService.getByID(id); // Chama o service para buscar usuários pelo ID
    ...
    static async update(req, res) {
        console.log('users_ProtectedController_update');
        try {
            const requestedID = parseInt(req.params.id); // ID passado na URL
            const requester = req.users; // Extraído do token JWT no middleware authenticateToken
            console.log('Usuário autenticado via JWT:', requester);
            ...
            const id = req.params.id; // Pega o ID da URL
            await usersService.update(id, req.body); // Chama o service para atualizar
    ...
    static async adminOnly_delete(req, res) {
        console.log('users_ProtectedController_adminOnly_update');
        try {
            const id = req.params.id; // Pega o ID da URL
            await usersService.delete(id); // Chama o service para deletar
...
module.exports = Users_ProtectedController;