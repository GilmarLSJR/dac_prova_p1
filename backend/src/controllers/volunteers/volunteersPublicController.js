...
class VolunteersPublicController {
  // Método para criar um novo usuário
  static async create(req, res) {
    console.log('volunteers_publicController_create');
    try {
      const result = await VolunteersService.create(req.body); // Chama o service para criar usuário
  ...
  static async login(req, res) {
    console.log('volunteers_publicController_login');
    try {
      // Chama o serviço para autenticar o usuário, passando os dados da requisição
      const result = await VolunteersService.login(req.body);
...
module.exports = VolunteersPublicController;
