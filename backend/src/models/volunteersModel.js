...
class VolunteersModel {
  ...
  static async findByID(id) {
    ...
    const [rows] = await db.query('SELECT * FROM volunteers WHERE id = ?', [
      id,
    ]);
    return rows[0];
  }
  ...
  static async create(volunteer) {
    ...
    const { name, email, password, phone } = volunteer;
    const [result] = await db.query(
      'INSERT INTO volunteers (name, email, password, phone) VALUES (?, ?, ?, ?)',
  ...
  static async update(id, volunteer) {
    ...
    const { name, email, password, phone, role } = volunteer;
    const [result] = await db.query(
  ...
  static async delete(id) {
    ...
    const [result] = await db.query('DELETE FROM volunteers WHERE id = ?', [
      id,
    ]);
    return result.affectedRows; // Retorna o número de linhas afetadas
  }
